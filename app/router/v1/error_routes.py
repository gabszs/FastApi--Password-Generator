...
async def bad_request_example(message: str = Query("BadRequestError: Requisição inválida. Verifique os parâmetros enviados.")):
    """
...
async def unauthorized_example(message: str = Query("InvalidCredentials: Credenciais inválidas. Verifique seu login e senha.")):
    """
...
async def forbidden_example(message: str = Query("AuthError: Acesso negado. Você não tem permissão para acessar este recurso.")):
    """
...
async def not_found_example(message: str = Query("NotFoundError: Recurso não encontrado.")):
    """
...
async def validation_error_example(message: str = Query("ValidationError: Erro de validação. Os dados enviados não são válidos.")):
    """
...
async def conflict_example(message: str = Query("DuplicatedError: Recurso já existe. Não é possível criar um duplicado.")):
    """
